// PLYLoader from Three.js r172 - adapted for global THREE namespace (r128 compat)
// Original: https://cdn.jsdelivr.net/npm/three@0.172.0/examples/jsm/loaders/PLYLoader.js
...
class PLYLoader {
	...
	parse( data ) {
		...
		function parseHeader( data, headerLength ) {
		...
		function mapElementAttributes( properties ) {
			...
			function findAttrName( names ) {
				for ( let i = 0, l = names.length; i < l; i ++ ) {
					if ( elementNames.includes( names[ i ] ) ) return names[ i ];
				}
				return null;
			}
		...
		function postProcess( buffer ) {
			...
			if ( buffer.faceVertexUvs.length > 0 || buffer.faceVertexColors.length > 0 ) {
				...
				if ( buffer.faceVertexUvs.length > 0 ) geometry.setAttribute( 'uv', new THREE.Float32BufferAttribute( buffer.faceVertexUvs, 2 ) );
				if ( buffer.faceVertexColors.length > 0 ) geometry.setAttribute( 'color', new THREE.Float32BufferAttribute( buffer.faceVertexColors, 3 ) );
			}
		...
		// r172 improved header extraction - byte-level, no full decode of binary data
...
// Export as global (compatible with THREE r128 global namespace)
